...
class _MarchesPageState extends State<MarchesPage> {
  ...
  Future<void> loadCryptoData() async {
    ...
    await rootBundle.loadString('assets/cryptocurrencies_db_detailed.json');
    ...
    setState(() {
      cryptoData = currencyHistory.entries.map((entry) {
        ...
        double change_24h =
            (lastEntry['price_usd'] - firstEntry['price_usd']) / firstEntry['price_usd'] * 100;
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        backgroundColor: Colors.grey[900], // Set AppBar background color to darker grey
        iconTheme: IconThemeData(color: Colors.white38),
      ),
      body: Container(
        color: Colors.grey[800], // Set background color to dark grey
        child: ListView.builder(
          ...
          itemBuilder: (context, index) {
            ...
            String priceUsdStr =
                latestData['price_usd']?.toStringAsFixed(2) ?? 'N/A';
            String change24hStr =
                crypto['change_24h']?.toStringAsFixed(2) ?? 'N/A';

            return ListTile(
              leading: crypto['icon'] != null ? Image(image: crypto['icon']) : null,
              title: Text(
                crypto['name'],
              ...
              subtitle: Text(
                '\$$priceUsdStr',
                style: TextStyle(color: Colors.lightBlueAccent), // Change text color to white
              ),
              trailing: Text(
                '$change24hStr%',
                style: TextStyle(
